...
export default function ConfirmationStep({ data }: { data: EntryData }) {
  ...
  return (
    <div className="space-y-6 slide-up">
      ...
      {/* Link do bolão para compartilhar */}
      <div
      ...
      >
        ...
        <button
          onClick={() => {
            navigator.clipboard?.writeText(BOLAO_URL);
          }}
          className="font-mono text-sm font-bold px-4 py-2 rounded-lg border w-full text-left truncate transition-all hover:opacity-80"
          style={{ color: "#FFD700", borderColor: "rgba(255,215,0,0.25)", background: "rgba(255,215,0,0.06)" }}
          title="Clique para copiar"
        >
          🔗 {BOLAO_URL.replace("https://", "")}
        </button>
        <p className="text-white/40 text-xs mt-2">Clique para copiar o link</p>
      </div>
